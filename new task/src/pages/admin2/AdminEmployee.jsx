...
import EditEmployeeModal from './EditEmployeeModals';
import AddEmployeeModal from '../admin2/AddEmployeeModals';
...
const openEditModal = (emp) => {
  ...
  const roleCounts = safeEmployees.reduce((acc, emp) => {
    const role = emp.emp_role || "Unknown";
    acc[role] = (acc[role] || 0) + 1;
    return acc;
  }, {});

  const teams = [...new Set(safeEmployees.map((emp) => emp.emp_role).filter(Boolean))];

const sortedEmployees = [...safeEmployees].sort((a, b) => {
  const numA = Number.parseInt(String(a.emp_id || '').replace(/\D/g, ''), 10) || 0;
  const numB = Number.parseInt(String(b.emp_id || '').replace(/\D/g, ''), 10) || 0;
  return numA - numB;
});
    ...
    <div className="dashboard-space-y">
         ...
         const RoleIcon = getRoleIcon(roleKey);
          return (
            <StatCard
              ...
              icon={RoleIcon}
              variant={roleVariants[roleKey] || "secondary"}
            />
          );
        })}
      </div>
      ...
      <Card className="ui-card-p6">
            ...
            {view === "Employee" && (
              ...
              >
                ...
                {safeEmployees.map((emp, index) => (
                  <option key={emp.emp_id ? `${emp.emp_id}-${index}` : index} value={emp.emp_name}>
                    {emp.emp_name}
                  </option>
                ))}
              </select>
        ...
        {(() => {
          ...
          if (error) return <ErrorState onRetry={() => fetchEmployees()} />;
          if (employees.length === 0) {
            return <EmptyState icon={Users} title="No employee records found" description="Try adding a new employee or adjusting your search." />;
          }
          return (
